...
from packages.correlation_package.correlation import Correlation
...
class FlowNetC(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('Conv3a Mean:', torch.mean(out_conv3a).cpu().numpy())
        # print('Conv3b Mean:', torch.mean(out_conv3b).cpu().numpy())
        # Merge streams
        out_corr = self.corr(out_conv3a, out_conv3b) # False
        # print('Corr Res', torch.mean(out_corr).cpu().numpy())
        out_corr = self.corr_activation(out_corr)
        # print(torch.mean(out_corr).cpu().numpy())
